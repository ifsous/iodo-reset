...
function calcSemaphore(
  ...
  // Vermelho imediato — palpitações
  if (symptoms.includes('palpitations')) return 'red'

  // Vermelho — dose alta + muitos sintomas
  if (drops >= 6 && realSymptoms.length >= 3) return 'red'

  // Amarelo — scores muito baixos
  if (energy <= 3 || mood <= 3) return 'yellow'

  // Amarelo — detox moderado
  if (realSymptoms.length >= 2) return 'yellow'

  // Amarelo — poucos cofatores
...
function SymptomChip({
  label, checked, onClick, danger = false,
}: { label: string; checked: boolean; onClick: () => void; danger?: boolean }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
export default function DiaryForm({
  userId, userName, recommendedDrops, existing, today,
}: Props) {
  const router  = useRouter()
  const supabase = createClient()

  // Estado inicial — pré-popula se existe registro do dia
  ...
  // Semáforo calculado em tempo real
  ...
  function toggleSymptom(val: SymptomType) {
    ...
    const withoutNone = symptoms.filter((s) => s !== 'none')
    setSymptoms(
      withoutNone.includes(val)
        ? withoutNone.filter((s) => s !== val)
        : [...withoutNone, val]
    )
  }
        ...
        <div className="max-w-lg mx-auto flex items-center gap-3">
          ...
          <div>
            <h1 className="text-white text-lg font-semibold">
              {isEditing ? 'Editar registro' : 'Registro de hoje'}
            </h1>
      ...
      <div className="max-w-lg mx-auto px-4 space-y-4 pt-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          ...
          <div className="flex items-center justify-center gap-6 py-3">
            ...
            <button
              type="button"
              onClick={() => setDrops((d) => Math.min(12, d + 1))}
              className="w-10 h-10 rounded-full border border-gray-200 bg-gray-50
                         flex items-center justify-center text-xl font-medium
                         text-gray-600 hover:bg-gray-100 active:scale-95 transition-all"
            >
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-5">
          ...
          <SliderField
            label="Energia"  value={energy} onChange={setEnergy}
            color={energy >= 7 ? 'text-teal-600' : energy >= 4 ? 'text-amber-500' : 'text-red-500'}
          />
          <SliderField
            label="Humor"    value={mood}   onChange={setMood}
            color={mood >= 7 ? 'text-purple-600' : mood >= 4 ? 'text-amber-500' : 'text-red-500'}
          />
          <SliderField
            label="Sono"     value={sleep}  onChange={setSleep}
            color={sleep >= 7 ? 'text-blue-600' : sleep >= 4 ? 'text-amber-500' : 'text-red-500'}
          />
        </div>
          ...
          <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mb-4">
            <div
              className={`h-full rounded-full transition-all duration-300 ${
                checklistPct >= 75 ? 'bg-emerald-500' :
                checklistPct >= 50 ? 'bg-amber-400'   : 'bg-red-400'
              }`}
                ...
                <div className={`w-5 h-5 rounded-md border flex items-center justify-center flex-shrink-0 transition-all ${
                  ...
                  {checks[key] && (
                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                      <path d="M1.5 5.5L4 8L8.5 3" stroke="white" strokeWidth="1.5"
                        strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  )}
                </div>
                <span className={`text-sm transition-colors ${
                  checks[key] ? 'text-gray-900 font-medium' : 'text-gray-600'
                }`}>
                  {label}
                </span>
              </button>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          ...
          <textarea
            ...
            className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm
                       ...
                       focus:ring-teal-500 focus:border-transparent resize-none"
          />
        </div>
        ...
        <button
          ...
          onClick={handleSave}
          disabled={saving}
          className="w-full py-4 bg-teal-700 hover:bg-teal-800
        ...
        >
          ...
          ) : isEditing ? 'Atualizar registro' : 'Salvar registro do dia'}
        </button>
